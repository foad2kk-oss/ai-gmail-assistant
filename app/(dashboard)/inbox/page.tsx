    ...
    <div className="flex h-full gap-4 min-h-0">
      <div className={`flex flex-col transition-all duration-300 ${selectedEmail ? 'w-full md:w-2/5 lg:w-1/3' : 'w-full'}`}>
        <div className="flex items-center gap-2 mb-4">
          ...
          <h1 className="text-lg font-bold text-white font-arabic">غير المقروءة</h1>
        </div>
        <div className="flex-1 overflow-y-auto">
          <EmailList folder="UNREAD" onSelectEmail={setSelectedEmail} selectedId={selectedEmail?.id} />
        </div>
      </div>
      <AnimatePresence>
        {selectedEmail && (
